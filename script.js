...
async function* gatherData() {
  ...
  async function displayData() {
    const dataGen = gatherData();

    console.log(await (await dataGen.next()).value);
    console.log(await (await dataGen.next()).value);
    console.log(await (await dataGen.next()).value);
}
...
let gene = countdownGenerator(5);
let nextValue = gene.next().value;
while(!gene.next().done) {
  console.log(nextValue);
  nextValue = gene.next().value;
}